...
void eval(char *cmdline) 
{
  ...
  /* If not a built-in command, fork and exec. */
  if(!builtin_cmd(argv)) 
  {
     ...
     * If a foreground job, addjob and wait for completion. 
     * Otherwise, addjob and output jobs list. 
     */
...
void sigint_handler(int sig) 
{
  ...
  if( pid = fgpid(jobs) > 0)
  {
    ...
    {
      printf("An error has occurred in the kill. \n");
    }
  }
  return;
}
...
void sigtstp_handler(int sig) 
{
  ...
  if(( pid = fgpid(jobs)) > 0 ) 
  {
    if( kill( -pid, sig ) < 0 )
    {
      printf("An error has occurred in the stop. \n");
    }
  }
}
